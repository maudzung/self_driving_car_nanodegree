...
def measure_curvature_real():
    ...
    left_curverad = np.sqrt((1 + (2 * left_fit_cr[0] * y_eval + left_fit_cr[1]) ** 2) ** 3) / np.absolute(
        2 * left_fit_cr[0])  ## Implement the calculation of the left line here
    right_curverad = np.sqrt((1 + (2 * right_fit_cr[0] * y_eval + right_fit_cr[1]) ** 2) ** 3) / np.absolute(
